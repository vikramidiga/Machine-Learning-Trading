...
class generalInfo{
	...
	public:
		...
		//copy constructor
		generalInfo(const generalInfo& x):code(x.code),stockLocate(x.stockLocate),trackingNumber(x.trackingNumber),
			timestamp(x.timestamp){}
		
		
		generalInfo(generalInfo&& x):code(x.code),stockLocate(x.stockLocate),trackingNumber(x.trackingNumber),timestamp(x.timestamp){}
		
		generalInfo& operator= (generalInfo&& x){
			code = x.code;
			stockLocate = x.stockLocate;
...
class xclusiveInfo{
	...
	xclusiveInfo(const xclusiveInfo& x):stock(x.stock),indicator(x.indicator),shares(x.shares),
		price(x.price),attribution(x.attribution),canceledShares(x.canceledShares){}
	xclusiveInfo(xclusiveInfo&& x):stock(x.stock),indicator(x.indicator),shares(x.shares),
		price(x.price),attribution(x.attribution),canceledShares(x.canceledShares){}	
	...
	xclusiveInfo& operator=(const xclusiveInfo& x)
	{
		...
		canceledShares = x.canceledShares;
		return *this;
	}
...
class order{
	...
	public:
		...
		void setShares(const long& shares){ xinfo.setShares(shares);}
		...
		order(const order& x):ginfo(x.ginfo),orderReferenceNumber(x.orderReferenceNumber),xinfo(x.xinfo),
					oldOrderReferenceNumber{x.oldOrderReferenceNumber}{}
		order(order&& x):ginfo(x.ginfo),orderReferenceNumber(x.orderReferenceNumber),xinfo(x.xinfo),
					oldOrderReferenceNumber{x.oldOrderReferenceNumber}{}
			...
			}else if(messageType=="X" || messageType=="E" || messageType=="C" )
			{
				...
				if(mp[t.getOrderReferenceNumber()])
				{
					...
					while(!bid.empty())
					{
						...
						{
							order a = bid.top();
							int shares = a.getXclusiveInfo().getShares() - t.getXclusiveInfo().getCanceledShares();
							if(shares>0)
							{
								a.setShares(shares);
								q.push(a);
								cout << "pushsed :" ;a.printOrder();cout << endl;
							}
